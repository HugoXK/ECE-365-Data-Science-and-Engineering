...
class Question1(object):
    def kMeans(self,data,K,niter):
        ...
        **For grading purposes only:**
        ...
        for i in range(niter):
            distances = dist.cdist(data,centers,'euclidean')
    ...
    def calculateJ(self,data):
        ...
        for K in range(2,11):
            ...
            for j in range(K):
                err[K-2]+=((data[labels==j]-centers[j])**2).sum()
...
class Question2(object):
    def trainVQ(self,image,B,K):
        ...
        # Put your code below
        N,M = image.shape
        img_flat = np.zeros((N*M//B**2,B**2))
        img_mtx = (image.reshape(N//B, B, -1, B).swapaxes(1,2).reshape(-1, B, B)) 
        for i in range(int((N*M)/(B*B))):
            img_flat[i] = img_mtx[i].flatten()
        kmeans = KMeans(n_clusters=K,init='k-means++').fit(img_flat)
    ...
    def compressImg(self,image,codebook,B):
        ...
        **For grading purposes only:**
        ...
        N,M = image.shape
        img_mtx = np.zeros((N*M//B**2,B**2))
        for i in range(N):
            for j in range (M):
                img_mtx[i//B*M//B+j//B][i%B*B+j%B]=image[i][j]
        idx = classifier.kneighbors(img_mtx,return_distance=False)
        ...
        **For grading purposes only:**
        ...
        for K in K_list:
            ...
            for i in range(num_label):
                cls = (trainlabels==labels[i])
                proto_lab[i*K:(i+1)*K] = np.repeat(labels[i],K)
                proto_dat[i*K:(i+1)*K,:] = KMeans(n_clusters=K,init='k-means++').fit(traindata[cls]).cluster_centers_
...
class Question4(object):
    ...
    def OLSRSS(self,trainfeat,trainresp,valfeat,valresp):
        ...
        rss = np.linalg.norm(est-valresp)**2.0/len(valresp)
        return rss
    ...
    def RidgeRSS(self,trainfeat,trainresp,valfeat,valresp):
        ...
        for i in range(len(a)):
            ...
            model.fit(trainfeat, trainresp) 
            est = model.predict(valfeat)
            rss_array[i] = np.linalg.norm(est-valresp)**2/len(valresp)

        cls = np.argmin(rss_array)
        best_a = a[cls]
        best_rss = rss_array[cls]
